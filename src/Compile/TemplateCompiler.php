...
class TemplateCompiler
{
    ...
    public function compile($template)
    {
        ...
        if ($file === false) {
            throw new InvalidTemplateException('Could not read template: ' . $template);
        }
        if (!$dom->load($file, true, false)) {
            throw new InvalidTemplateException('Could not parse template: ' . $template);
        }
        ...
        foreach ($node->macros as $macro => $value) {
            ...
            if (!isset($this->macros[$macro])) {
                throw new InvalidMacroException(
                    'Undefined macro "' . $macro . '" in template: ' . $this->currentTemplate
                );
            }
